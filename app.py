...
def DiscordWebhookMessage(message):
    ...
    newActivity = AdminActivity(time=int(time.time()), message=message)
...
class Admin(UserMixin, db.Model):
    username: Mapped[str] = mapped_column(
        db.String(80), primary_key=True, unique=True, nullable=False
    )
...
def AdminHome():
    ...
    for log in adminActivity:
        log.time = datetime.fromtimestamp(log.time, tz=timezone.utc).strftime(
            "%Y-%m-%d %H:%M:%S UTC"
        )
...
def AdminVersions():
    ...
    if request.method == "GET":
        return render_template(
            "admin_versions.html",
            pc_version=open("data/persist/version.txt", "r").read(),
...
def AdminServer():
    ...
    if last_backup_file == "":
        ...
        last_backup = time_ago_string(
            datetime.strptime(last_backup_file, "%Y-%m-%d_%H-%M-%S")
        )
...
def AdminPlayers():
    ...
    players = [
        ...
        if player["game"] != None and player["leader_level"] != None
    ]
    ...
    for player in players:
        player["last_online"] = datetime.fromtimestamp(
            player["last_online"], tz=timezone.utc
    ...
    return render_template(
        "admin_players.html", players=players, player_count=len(players)
    )
...
def AdminPlayer(player):
    ...
    for battle in battle_history:
        battle["recordTime"] = datetime.fromtimestamp(
            battle["recordTime"], tz=timezone.utc
        ).strftime("%Y-%m-%d %H:%M:%S UTC")

    # fix device name
    if player["devicename"] is not None:
        player["devicename"] = re.sub(
            r"%[0-9A-Fa-f]{2}",
            lambda m: chr(int(m.group(0)[1:], 16)),
            player["devicename"],
        )
    ...
    return render_template(
        "admin_player.html",
        player=player,
        is_banned=IsUserBanned(player["username"]),
        ...
        InstalledDate=datetime.fromtimestamp(
            game["InstalledDate"], tz=timezone.utc
        ).strftime("%Y-%m-%d %H:%M:%S UTC"),
        PVPBanned=bool(game["Zxcvbnm"]),
        MultiplayerLevel=game["MultiplayerLevel"],
        InventorySpace=game["InventorySpace"],
...
def AdminPlayerGame(player):
    ...
    return render_template(
        "admin_player_game.html", game=game, player_id=player["username"]
    )
...
def AdminPlayerGameEdit(player):
    ...
    Log(
        "admin",
        current_user.username + " edited game data for player: " + player.username,
    )
...
def AdminPlayerAction(player, action):
    ...
    DiscordWebhookMessage(
        current_user.username + " performed " + action + " on ID: " + player
    )
...
def SystemBan(username):
    ...
    if not IsUserBanned(username):
        newban = Bans(
            username=username, bantype="userid", author="SYSTEM", time=int(time.time())
        )
        db.session.add(newban)
        db.session.commit()
...
def AdminIPBanAction():
    ...
    DiscordWebhookMessage(
        current_user.username + " performed ban on IP: " + request.form["ip"]
    )
    return redirect("/admin/bannedips")
...
def AdminBannedPlayers():
    ...
    # get player name
    for ban in bans:
        ban["multiplayer_name"] = (
            Player.query.filter_by(username=ban["username"]).first().multiplayer_name
        )
        if ban["multiplayer_name"] is None:
            ban["multiplayer_name"] = GetNameFromSave(
                Player.query.filter_by(username=ban["username"]).first().game
            )
        if ban["time"] is not None:
            ban["time"] = datetime.fromtimestamp(ban["time"], tz=timezone.utc).strftime(
                "%Y-%m-%d %H:%M:%S UTC"
            )
...
def Backup():
    ...
    # copy database
    shutil.copy(f"{DB_PATH}/cardwarskingdom.db", f"backup/{now}/cardwarskingdom.db")
...
def AdminLogsDeleteOlderThan(days):
    ...
    Log(
        "admin",
        current_user.username + " deleted logs older than " + str(days) + " days",
    )
...
def AdminUpsightDeleteOlderThan(days):
    ...
    # delete logs older than x days
    db.session.query(UpsightLogs).filter(
        UpsightLogs.time < int(time.time()) - seconds
    ).delete()
    db.session.commit()
    ...
    Log(
        "admin",
        current_user.username
        + " deleted upsight logs older than "
        + str(days)
        + " days",
    )
...
def AdminLogs():
    ...
    logs = db.paginate(
        db.select(Logs).order_by(Logs.id.desc()), page=pagerequest, per_page=perpage
    )

    if query != "":
        logs = db.paginate(
            db.select(Logs).filter(Logs.player == query).order_by(Logs.id.desc()),
            page=pagerequest,
            per_page=perpage,
        )
...
def AdminUpsight():
    ...
    query = request.args.get("query", "", type=str)
    logs = db.paginate(
        db.select(UpsightLogs).order_by(UpsightLogs.id.desc()),
        page=pagerequest,
        per_page=perpage,
    )
    ...
    if query != "":
        logs = db.paginate(
            db.select(UpsightLogs)
            .filter(UpsightLogs.player_id == query)
            .order_by(UpsightLogs.id.desc()),
            page=pagerequest,
            per_page=perpage,
        )
...
def PersistMessagesReceivedIDs():
    return send_from_directory(
        directory="",
        path="data/persist/messages_received_ids.json",
        as_attachment=True,
...
def PersistMessagesGet(message):
    ...
    return send_from_directory(
        directory="",
        path=f"data/persist/messages/{message}.json",
        as_attachment=True,
...
def AccountPreAuth():
    data = {"data": {"nonce": os.urandom(32).hex()}}
    return jsonify(data)
...
def AccountGCAuth():
    ...
    if db_user is None:
        ...
        PlayerLog(
            ...
            player=clientData["player_id"],
            message="Created new player",
        )

    data = {"data": {"user_id": clientData["player_id"], "is_new": isplayernew}}
    return data
...
def MultiplayerUpdateDeckName():
    ...
    if db_user.leader_level is None and int(clientData["leader_level"]) > 5:
        DiscordWebhookMessage(
            f"{clientData['player_id']} attempted to set leader level to a {clientData['leader_level']} on a fresh account! IP: "
            + IPFromRequest(request)
        )
        SystemBan(clientData["player_id"])
    elif (
        db_user.leader_level is not None
        and int(clientData["leader_level"]) > int(db_user.leader_level) + 10
    ):  # Make sure leader level is incremented by more than 10, if not, ban user
        DiscordWebhookMessage(
            f"{clientData['player_id']} attempted to set leader level to {clientData['leader_level']} when it was set to {db_user.leader_level}! IP: "
            + IPFromRequest(request)
        )
        SystemBan(clientData["player_id"])
...
def get_hash_string(source_value, key):
    hmac_sha256 = hmac.new(
        key.encode("utf-8"), source_value.encode("utf-8"), hashlib.sha256
    )
...
def PersistGame():
    ...
    if request.headers.get("Player-Id") is None:
        DiscordWebhookMessage(
            "User attempted to access game without Player-Id header. IP: "
            + IPFromRequest(request)
        )
        abort(404)
    ...
    if request.headers.get("Age") is None:
        DiscordWebhookMessage(
            username
            + " attempted to access game without Age header. IP: "
            + IPFromRequest(request)
        )
        abort(404)
    if request.headers.get("User-Agent") != "Innertube Explorer v0.1":
        DiscordWebhookMessage(
            username
            + " attempted to access game without User-Agent header. IP: "
            + IPFromRequest(request)
        )
        abort(404)
    if request.headers.get("Platform") is None:
        DiscordWebhookMessage(
            username
            + " attempted to access game without Platform header. IP: "
            + IPFromRequest(request)
        )
        abort(404)
    if request.headers.get("Version") is None:
        DiscordWebhookMessage(
            username
            + " attempted to access game without Version header. IP: "
            + IPFromRequest(request)
        )
        abort(404)
    if request.method == "PUT":
        if request.headers.get("X-Nick-Description") is None:
            DiscordWebhookMessage(
                username
                + " attempted to access game without X-Nick-Description header. IP: "
                + IPFromRequest(request)
            )
            abort(404)
    ...
    if request.method == "PUT":
        ...
        if DeviceNameUser.devicename != devicename:
            DiscordWebhookMessage(
                username
                + " attempted to access game with wrong device name. Device name: '"
                + devicename
                + "'. IP: "
                + IPFromRequest(request)
            )
        ...
        try:
            ...
            if game is not None:
                if int(game["Zxcvbnm"]) == 1:
                    DiscordWebhookMessage(
                        username
                        + " attempted to access game while PVP banned. IP: "
                        + IPFromRequest(request)
                    )
                    SystemBan(username)
                    return make_response("User is banned!", 400)
    ...
    if request.method == "PUT":
        ...
        if not data.startswith(b"username=") or data.startswith(b"{"):
            DiscordWebhookMessage(
                username
                + " attempted to put game data without encryption. IP: "
                + IPFromRequest(request)
                + ". Data: "
                + data.decode("utf-8")[:50]
            )
...
def PersistFriendsFindCandidates():
    ...
    data2 = {"success": True, "data": json.dumps(data)}
...
def PersistFriendsRequestWithMyInfo():
    ...
    try:
        invite_user = Player.query.filter_by(
            username=clientData["invite_id"].replace("_", "-")
        ).first()
    except:
...
def GetAllyInfo(player_id: str, isally: bool):
    ...
    data = {
        "fields": {
            ...
            "anonymoushelpcount": db_user.anonymoushelpcount
            if db_user.anonymoushelpcount is not None
            else "0",
...
@app.route(
    "/persist/friends_deny_request/<string:player_id>/<string:invite_id>",
    methods=["GET"],
)
...
@app.route(
    "/persist/friends_remove/<string:player_id>/<string:invite_id>", methods=["GET"]
)
...
def AnalyticsUpsight():
    ...
    if (
        headers.get("Player-Id") is None
        or headers.get("Event-Type") is None
        or headers.get("Event-Action") is None
    ):
    ...
    if headers.get("Event-Action") == "detector":
        DiscordWebhookMessage(
            headers.get("Player-Id") + " triggered ACTk Anti-Cheat. Data:" + message
        )
...
def AnalyticsPVPMatch():
    ...
    with open(
        "data/persist/pvpmatches/"
        + headers.get("Player-Id", "unknown")
        + "_"
        + headers.get("Match-Id", "unknown")
        + ".json",
        "w",
    ) as outfile:
...
def LeaderboardFetchEntries():
    ...
    for player in allplayers:
        if (
            player.multiplayer_name is None
            or player.multiplayer_name == b""
            or player.multiplayer_name == b" "
        ):
            continue
    ...
    return jsonify({"success": True, "data": f"{json.dumps(leaderboard)}"})
...
def GetPlayerWins(player_id):
    ...
    with open("data/persist/blueprints/db_PVPSeasons.json") as f:
        ...
        for season in seasons:
            ...
            if (
                int(time.time())
                < datetime.strptime(season["EndDate"], "%m/%d/%Y").timestamp()
            ):
